...
def dummy_data_init(number_of_workers, data_number, pretrain = False, true_label = None):
    ...
    if pretrain:
        ...
        dummy_images = []
        for worker_index in range(number_of_workers):
            temp_dummy_image = tf.random.uniform(shape=[data_number, 7, 7], seed= worker_index + 1)
...
def take_gradient(number_of_workers, random_lists, real_data, real_labels, local_net, server):
    ...
    with tf.GradientTape(persistent = True) as tape:
        ...
        for worker_index in range(number_of_workers):
            ...
            temp_data = tf.reshape(temp_data, [-1, 7, 7,1])
...
class Optimizer_for_cafe():
    ...
    def __init__(self, number_of_workers, data_number, learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-7):
        ...
        for worker_index in range(number_of_workers):
            self.h_data.append(tf.Variable(tf.zeros([data_number, 7, 7])))
            self.v_data.append(tf.Variable(tf.zeros([data_number, 7, 7])))
    ...
    if real:
        ...
        for worker_index in range(number_of_worker):
            ...
            for data_index in range(data_number):
                # Initialize folder
                save_path = 'result/{}'.format(str(worker_index))
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                # change image format
                data_to_be_visualized = data[worker_index][data_index, :, :].numpy()
                data_to_be_visualized = tf.reshape(data_to_be_visualized, [7, 7])
                plt.imshow(data_to_be_visualized)
                plt.savefig(os.path.join(save_path, 'real{}'.format(str(data_index))))
                plt.close()
        print("Done!")
    else:
        # save dummy iamge
        print("saving dummy images!")
        for worker_index in range(number_of_worker):
            data_number = data[worker_index].numpy().shape[0]
            for data_index in range(data_number):
                # Initialize folder
                ...
                data_to_be_visualized = tf.reshape(data_to_be_visualized, [7, 7])
                plt.imshow(data_to_be_visualized)
                plt.savefig(os.path.join(save_path, 'dummy{}'.format(str(data_index))))
                plt.close()
        print("Done!")
...
def PSNR(batch_real_image, batch_dummy_image):
    ...
    for worker_index in range(len(batch_real_image)):
        dummy = tf.reshape(tf.clip_by_value(batch_dummy_image[worker_index], 0, 1), [-1, 7, 7, 1])
        real = tf.reshape(batch_real_image[worker_index], [-1, 7, 7, 1])

        psnr_list = tf.image.psnr(real, dummy, 1.0).numpy()
        psnr_list[np.isinf(psnr_list)] = 40
        psnr.append(np.mean(psnr_list))
    aggregated_psnr = np.mean(psnr)
    print('psnr list: ',psnr, end='\t')
    print('psnr value:', aggregated_psnr, end='\t')
    return aggregated_psnr
