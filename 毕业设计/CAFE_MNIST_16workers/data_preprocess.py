...
def load_data(data_path):
    ...
    # train_data = tf.image.resize(train_data, [56,56])
    #
    # test_images = np.expand_dims(test_images, axis=-1)
    # test_images = test_images.astype('float32')/255
    # test_images = tf.image.resize(test_images, [56,56])
    print(train_data.shape, test_images.shape)
    ...
    for worker_index in range(16):
        i = worker_index // 4
        j = worker_index % 4
        slice = train_data[:, 7 * i: 7 * (i + 1), 7 * j: 7 * (j + 1)]
    ...
    for worker_index in range(16):
        i = worker_index // 4
        j = worker_index % 4
        slice = test_data[:, 7 * i: 7 * (i + 1), 7 * j: 7 * (j + 1)]
