...
def load_data(data_path):
    ...
    for worker_index in range(16):
        i = worker_index // 4   #一张图分成四张图，分给四个client
        j = worker_index % 4    #将一张 32X32 的图像按左上、右上、左下、右下分割为四张 14X14 图片
        slice = train_data[:, 8 * i: 8 * (i + 1), 8 * j: 8 * (j + 1)]
    ...
    for worker_index in range(16):
        i = worker_index // 4
        j = worker_index % 4
        slice = test_data[:, 8 * i: 8 * (i + 1), 8 * j: 8 * (j + 1)]
